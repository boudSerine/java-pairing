...
public class TatePairingEXE {
	...
	public static void main(final String[] args) {
		...
		BigInteger pow = null;
		BigInteger invPow = null;

		if ((m % 8 == 1) | (m % 8 == 7)) {
			if (bE == 0) {
				pow = BigInteger.ZERO.setBit(3 * m).setBit((m + 1) / 2).setBit(
						2 * m);
				invPow = BigInteger.ZERO.setBit(m).setBit((5 * m + 1) / 2)
						.setBit(0);
			} else if (bE == 1) {
				pow = BigInteger.ZERO.setBit(3 * m).setBit((5 * m + 1) / 2)
						.setBit(2 * m);
				invPow = BigInteger.ZERO.setBit(m).setBit((m + 1) / 2)
						.setBit(0);
			}
		} else if ((m % 8 == 3) | (m % 8 == 5)) {
			if (bE == 1) {
		...
		// final BigInteger x = BigInteger.probablePrime(240, new Random());
		// final BigInteger y = BigInteger.probablePrime(240, new Random());
		...
		final TatePairing pairing = new TatePairing(r, field, curve, pow,
				invPow);
